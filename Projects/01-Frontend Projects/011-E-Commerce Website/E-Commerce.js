...
function addProductsToDOM() {
  ...
  productList.forEach((product) => {
    ...
    div.innerHTML = `<div class="product"  id=${product.id}>
        <div class="img_con">
        <img src=${product.images[0]}  id="0"></img>
        </div>
...
cartBtn.addEventListener("click", () => {
  ...
  document.querySelector(".products").style.display = cart.classList.contains(
    "show"
  )
    ? "none"
    : "flex";
...
document.getElementById("checkoutBtn").addEventListener("click", () => {
  if (cartItems.length === 0) window.alert(`Please add product to Cart !`);
  else
    window.alert(
      `Thankyou for shopping. Your total price is ${totalPrice.toFixed(2)}`
    );
});
