...
async function fetchStocks() {
    try {
        let response1 = await fetch(`https://stocks3.onrender.com/api/stocks/getstocksdata`);
        if (!response1.ok) {
        ...
        let response2 = await fetch(`https://stocks3.onrender.com/api/stocks/getstocksprofiledata`);
        if (!response2.ok) {
            throw new Error('Failed to fetch Stocks Data');
        }
        let data2 = await response2.json();
        if (!data2 || !data2.stocksProfileData || !Array.isArray(data2.stocksProfileData)) {
            throw new Error('Invalid data format of Stocks Data');
        }
        stocksProfileData = data2.stocksProfileData[0];

        let response3 = await fetch(`https://stocks3.onrender.com/api/stocks/getstockstatsdata`);
        if (!response3.ok) {
            throw new Error('Failed to fetch Stocks status');
        }
        let data3 = await response3.json();
        if (!data3 || !data3.stocksStatsData || !Array.isArray(data3.stocksStatsData)) {
...
function addStockBtn(stock){
    ...
    span.innerHTML = `
    <span style="color:${(stocksStatsData[stock]['profit']>0)?'green':'red'}">${stocksStatsData[stock]['profit'].toFixed(2)}%</span>
...
function updateStockInfo(){
    ...
    <span id='profit' style="color:${(stocksStatsData[currentStock]['profit']>0)?'green':'red'}">${stocksStatsData[currentStock]['profit']}%</span>
    <span id='bookValue'>$${stocksStatsData[currentStock]['bookValue']}</span>`;

    document.querySelector('#summary').textContent = stocksProfileData[currentStock]["summary"];
}
