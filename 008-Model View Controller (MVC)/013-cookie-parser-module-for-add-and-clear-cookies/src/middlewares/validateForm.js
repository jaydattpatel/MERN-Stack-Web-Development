...
export const validateForm = async (req,res,next) => {
    ...
    if(req.url == '/add-product'){
      rules.push(body('imageUrl').custom((value,{req})=>{
        if(!req.file){
    ...
    if (errors.length > 0) {
      if(req.url == '/add-product'){
        return res.render('add-product', {errorMessage: errors, userEmail: req.session.userEmail});
      }
      if(req.url == '/update-product'){
        const products = ProductModel.getById(req.body.id);
        return res.render('update-product',{errorMessage: errors, products: products, userEmail: req.session.userEmail})
      }
    }else{
        next();
    }
